...
class VigenciasWindow:
    ...
    def __init__(self, parent: tk.Tk, usuario: str = "", rol: str = ""):
        ...
        set_responsive_geometry(self.window, 1200, 650)
    ...
    def _build_ui(self) -> None:
        ...
        attach_treeview_tooltip(self.tree, {"cantidad", "nombre", "proveedor"})
        self.tree.tag_configure("vencido", background="#EF9A9A")
        self.tree.tag_configure("por_vencer", background="#FFF9C4")
        ...
        numeric_cols = {"cantidad", "dias"}
        for col in columns:
            self.tree.heading(col, text=headings[col], command=lambda c=col: self._sort_column(c, False))
            self.tree.column(
                col, width=widths[col], minwidth=max(60, int(widths[col] * 0.5)),
                anchor="e" if col in numeric_cols else "w", stretch=True,
            )
    ...
    def load_table(self) -> None:
        ...
        for idx, (row, record) in enumerate(filtered_rows[start:end]):
            estado_tag = estado_tags.get(str(row[8]), "")
            tags = (zebra_tag(idx),) + ((estado_tag,) if estado_tag else ())
            item_id = self.tree.insert("", tk.END, values=row, tags=tags)
